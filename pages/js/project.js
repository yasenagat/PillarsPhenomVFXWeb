...
require(['domready', 'jquery', 'validate'], function(doc, $,ld) {
    doc(function() {
		...
		$(".pro").on("click",".cz",function(){
			...
			var kssj = thistab.find(".StartDatetime_ div").html().trim();
			var jssj = thistab.find(".EndDatetime_ div").html().trim();
				...
				projectsave_ajax(action, pc, pn, pi, pl, pt, sd, ed, pd, function(data) {
					if(data.FeedbackCode == 0) {
						if(flag == "add"){
							...
							$(".tab"+pc+" .ProjectLeader_").html(pl);
							$(".tab"+pc+" .ProjectType_").html(pt);
							$(".tab"+pc+" .StartDatetime_ div").html(sd);
							$(".tab"+pc+" .EndDatetime_ div").html(ed);
							$(".tab"+pc+" .ProjectDetail_").html(pd);
							againjs();
							$(".outer").hide(500);
							$(".formdiv").hide(500);
						}
		...
		$(".inptext").blur(function(){
				$(".spsearch").find(".inpsearch").css("background","url('../img/glass.png')");
		});
		//得到焦点
		$(".inptext").focus(function(){
			$(".spsearch").find(".inpsearch").css("background","url('../img/glass2.png')");
		});
	});
});
