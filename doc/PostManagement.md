...
#####（3）素材的导入 #####
指定服务器上的一个目录，将目录下包含的所有拍摄素材的信息导入到数据库，不同类型的素材有不同的组织方式，按照素材的组织方式将素材一一导入，一个素材对应管理系统中的一条记录。

素材的类型：
* R3D，以.r3d为后缀的文件夹，文件夹中包含视频文件

#####（4）素材的导出 #####
将数据库中的素材下载下来，以供他用（要支持批量下载）
选择要下载的素材，指定目录下载
下载时选择需要的格式：原始，DPX，mov

素材的分组
    素材以组的形势进行分类，组具有树形结构，同一素材可以在不同的组中出现，对组对修改不会对素材造成任何影响，删除组也不会删除素材。
   默认创建All组

#####（5）创建组（支持模版功能）#####
点击创建按钮，弹出创建素材组对话框，输入素材组的名称，从下拉菜单中选择素材组的类型，填写标签信息用于标注文件夹的作用，点创建按钮创建素材组。
...
#####（10）快速查找功能
模糊查找，通过文件名，长度，元数据中的信息等进行查找，提供查找格式

#####（11）排序
根据素材的属性进行排序，大小，长度，修改日期等。

#####（12）素材转换功能
将素材转成需要的格式导出，转好的格式需保留，供再次使用，转换比较耗时，如果需要可在后台做集中转码处理，转换后提供下载链接。
导出jpg
导出DPX
导出mov
