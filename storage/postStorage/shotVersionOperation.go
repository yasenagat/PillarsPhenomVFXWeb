...
//func QueryShotVersionByShot(code *string) ([]utility.ShotVersion, error) {
//	stmt, err := mysqlUtility.DBConn.Prepare("SELECT version_code,shot_code,vendor_code,thumbnail,demo,product,status FROM shot_version WHERE shot_code=?  AND status=0")
//	defer stmt.Close()
